...
import { CalendarDays, MapPin, ArrowLeft, Bookmark, Clock, Users, CheckCircle, Minus, Plus } from 'lucide-react'
          ...
          <div className='flex-1'>
            {expo.type && (
              <span className={`inline-block text-xs font-bold px-3 py-1 rounded-full
                               ...
                               ${expo.type === 'concert' ? 'bg-purple-500/20 text-purple-300'
                  : expo.type === 'sports' ? 'bg-green-500/20 text-green-300'
                    : 'bg-blue-500/20 text-blue-300'}`}>
                {expo.type}
              </span>
            )}
            <h1 className='text-2xl md:text-4xl font-bold text-white mb-2'>{expo.title}</h1>
            {expo.artist && <p className='text-[#FFA641] font-semibold mb-3'>{expo.artist}</p>}
            {expo.team && <p className='text-[#FFA641] font-semibold mb-3'>{expo.team}</p>}
            ...
            <div className='flex gap-5 text-white/60 text-sm flex-wrap'>
              ...
              <span className='flex items-center gap-1.5'>
                <MapPin className='w-4 h-4' />{expo.location}{expo.city ? `, ${expo.city}` : ''}
              </span>
              {totalCap > 0 && (
                <span className='flex items-center gap-1.5'>
                  <Users className='w-4 h-4' />{totalSold} / {totalCap} registered
                </span>
              )}
            </div>
          </div>

          <div className='flex flex-col sm:flex-row items-start sm:items-center gap-3 flex-shrink-0 mt-4 md:mt-0'>
            ...
            {isSoldOut ? (
              ...
              <button onClick={() => setActiveTab('Tickets')}
                className='bg-[#FFA641] text-[#2C3E50] font-bold text-sm px-5 py-2.5
